...
/**
 * Brand kit context type for system prompt injection.
 * Defined inline to avoid circular dependency with shared/brand.
 */
interface BrandPromptOptions {
  /** Current TSX source code to inject as context */
  currentTsx?: string
  /** Brand kit prompt section (generated by generateBrandPromptSection) */
  brandSection?: string
}
 ...
 *
 * @param options - Context options including current TSX and brand kit
 * @returns The full system prompt with context
 */
export function getSystemPrompt(options?: BrandPromptOptions | string): string {
  // Handle legacy single-string signature for backward compatibility
  const opts: BrandPromptOptions =
    typeof options === 'string' ? { currentTsx: options } : (options ?? {})

  let prompt = SYSTEM_PROMPT

  // Inject brand kit section if provided
  if (opts.brandSection) {
    prompt = `${prompt}
...
${opts.brandSection}`
  ...
  // Inject current TSX code if provided
  if (opts.currentTsx) {
    prompt = `${prompt}
